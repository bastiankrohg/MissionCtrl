...
# Clear the terminal and set up dashboard
setup_dashboard() {
    clear
    echo "------------------------------"
    ...
    echo "Last Checked: N/A"
    echo ""
    echo "Next Check In: N/A"
    echo "------------------------------"
}
...
update_dashboard() {
    ...
    # Status
    tput cup 5 8  # Move to "Status:" position
    echo -n "                       "  # Clear the line
    echo -n "${color}${status}${RESET}"

    # Last Checked
    tput cup 6 14  # Move to "Last Checked:" position
    echo -n "                       "  # Clear the line
    echo -n "$(date '+%Y-%m-%d %H:%M:%S')"

    # Next Check In
    tput cup 8 15  # Move to "Next Check In:" position
    echo -n "                       "  # Clear the line
    echo -n "${next_check}s"
}
